...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => rufers_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => rufers_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => rufers_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => rufers_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => rufers_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => rufers_meta( 'banner_elementor_template', $page_id ),
				'banner'            => rufers_set( rufers_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => rufers_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => rufers_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : rufers_the_title( $template ),
				'banner'        => rufers_set( $bg, 'url' ),
